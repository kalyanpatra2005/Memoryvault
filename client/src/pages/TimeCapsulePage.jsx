...
export default function TimeCapsulePage() {
  ...
  const handleCreateCapsule = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.error || 'Failed to seal capsule');
      }
  ...
  const handleOpenCapsule = async (id) => {
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setCapsules(capsules.map(c => c.id === id ? data.capsule : c));
        setOpenedCapsule(data.capsule);
      } else {
        alert(data.error || 'Could not unseal capsule yet.');
      }
    } catch (err) {
  ...
  const handleDeleteCapsule = async (id, e) => {
    ...
    try {
      const res = await authFetch(`/api/capsules/${id}`, { method: 'DELETE' });
      if (res.ok) {
